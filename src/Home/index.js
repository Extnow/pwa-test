...
export const Home = () => {
  const [isShownInstallPopup, toggleInstallPopup] = useState(false);
  const toggle = () => toggleInstallPopup(!isShownInstallPopup);

  useEffect(() => {
    isIos() && !isInStandaloneMode() && toggleInstallPopup(true);
  }, []);
  ...
  return (
    <Container>
      ...
      {isShownInstallPopup && <InstallPopup toggle={toggle} />}
      ...
      <Description>
        ...
        нажать на кнопку "Отправить" и в появившемся меню выбрать пункт "на экран домой" (работает
        не на всех версиях операционной системы)
      </Description>
      <ImgWrapper>
